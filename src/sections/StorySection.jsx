        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-12">
          ...
          >
            ...
            <div className="mt-5 space-y-3">
              {siteContent.story.highlights.map((item) => (
                <div
                  key={item.title}
                  className="rounded-2xl border border-cyan-200 bg-slate-50 px-4 py-3"
                >
          ...
          >
            {siteContent.story.stats.map((stat) => (
              <div
                key={stat.label}
                className="rounded-3xl border border-cyan-300/40 bg-white p-6 shadow-[0_0_40px_rgba(56,189,248,0.16)]"
              >
                <div className="text-2xl font-semibold text-cyan-600">{stat.value}</div>
